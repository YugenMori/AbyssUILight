...
--
-- NewUI (dragonflight+) module for AbyssUIClassic
--------------------------------------------------------------
-- Init - Tables - Saves
...
---------------------------- NewUI Functions ----------------------------------
local function AbyssUIClassic_TooltipSetUnit() 
    -- OnTooltipSetUnit
...
f:SetScript("OnEvent", function(self, event, name)
 if ((event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_FOCUS_CHANGED" or event == "PLAYER_ENTERING_WORLD" or event == "PLAYER_LOGIN") and GetWoWVersion >= 90500) then
    for i, v in pairs({ 
      ...
      if AbyssUIClassicAddonSettings ~= nil and AbyssUIClassicAddonSettings.GreenHealth ~= true then
        v:SetStatusBarTexture("Interface\\AddOns\\AbyssUIClassic\\textures\\Raid-Bar-Hp-Fill")
